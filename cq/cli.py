...
def cmd_add(args: argparse.Namespace) -> int:
    path = _db_path(args)
    task = store.add_task(args.description, context_policy=args.context_policy, path=path)
...
def cmd_list(args: argparse.Namespace) -> int:
    ...
    print(f"{'ID':<6} {'Status':<12} {'Created':<20} {'Description'}")
    print("-" * 70)
    for t in tasks:
        ...
        if len(desc) > 45:
            desc = desc[:42] + "..."
        print(f"{t['id']:<6} {t['status']:<12} {created:<20} {desc}")
    return 0
